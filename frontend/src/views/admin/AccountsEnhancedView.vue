    ...
    <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      ...
      <div v-else class="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        >
          ...
          <div class="mb-4">
            <h3 class="mb-2 text-sm font-semibold text-gray-700 dark:text-gray-300">
              {{ t('admin.accountsEnhanced.reasoningEffort') }}
            </h3>
            <div class="mb-2 flex items-center gap-3">
              <select
                v-model="account.config.reasoning_effort.mode"
                class="input input-sm w-40"
                @change="markDirty(account.id)"
              >
                <option value="off">{{ t('admin.accountsEnhanced.modes.off') }}</option>
                <option value="override">{{ t('admin.accountsEnhanced.modes.effortOverride') }}</option>
                <option value="fill">{{ t('admin.accountsEnhanced.modes.effortFill') }}</option>
              </select>
              <span v-if="account.config.reasoning_effort.mode !== 'off'" class="text-xs text-gray-500">
                {{
                  account.config.reasoning_effort.mode === 'override'
                    ? t('admin.accountsEnhanced.effortOverrideHint')
                    : t('admin.accountsEnhanced.effortFillHint')
                }}
              </span>
            </div>

            <template v-if="account.config.reasoning_effort.mode !== 'off'">
              <div
                v-for="(rule, idx) in account.config.reasoning_effort.rules || []"
                :key="idx"
              ...
              >
                <input
                  ...
                  :placeholder="t('admin.accountsEnhanced.effortModelPlaceholder')"
                  @input="markDirty(account.id)"
                />
                <span class="text-gray-400">→</span>
                <select
                  ...
                  @change="markDirty(account.id)"
                >
                  <option v-for="value in effortOptions" :key="value" :value="value">
                    {{ value }}
                  </option>
                </select>
                <button
                  ...
                  @click="removeEffortRule(account.id, idx)"
                >
                  {{ t('admin.accountsEnhanced.removeRule') }}
                </button>
              </div>
              <div v-if="!account.config.reasoning_effort.rules?.length" class="mb-2 text-xs text-gray-400">
                {{ t('admin.accountsEnhanced.noRules') }}
              </div>
              <button
                type="button"
                class="btn btn-secondary px-3 py-1 text-xs"
                @click="addEffortRule(account.id)"
              >
                {{ t('admin.accountsEnhanced.addRule') }}
              </button>
              <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                {{ t('admin.accountsEnhanced.effortHint') }}
              </p>
            </template>
          </div>
...
/** 可注入的思考强度档位（与后端白名单一致） */
const effortOptions = ['minimal', 'low', 'medium', 'high', 'xhigh', 'max']
...
function defaultConfig(): EnhancedControl {
  return {
    ...
    reasoning_effort: { mode: 'off', rules: [] } as ReasoningEffortSetting,
  }
}

function normalizeConfig(raw?: Partial<EnhancedControl> | null): EnhancedControl {
  const fallback = defaultConfig()
  return {
    cache_rate: { ...fallback.cache_rate, ...(raw?.cache_rate ?? {}) },
    ttft: { ...fallback.ttft, ...(raw?.ttft ?? {}) },
    reasoning_effort: {
      mode: raw?.reasoning_effort?.mode || 'off',
      rules: Array.isArray(raw?.reasoning_effort?.rules) ? raw.reasoning_effort.rules : [],
    },
  }
}
  ...
  try {
    ...
    accounts.value = await Promise.all(
      items.map(async (acc) => {
        ...
        try {
          config = normalizeConfig(await getEnhancedControl(acc.id))
        } catch {
          // keep default
        }
        dirty[acc.id] = false
...
async function saveAccount(id: number) {
  ...
  try {
    ...
    await updateEnhancedControl(id, {
      ...
      reasoning_effort: acc.config.reasoning_effort,
    })
    dirty[id] = false
  } finally {
    saving[id] = false
...
function addEffortRule(id: number) {
  const acc = accounts.value.find((a) => a.id === id)
  if (!acc) return
  if (!Array.isArray(acc.config.reasoning_effort.rules)) {
    acc.config.reasoning_effort.rules = []
  }
  acc.config.reasoning_effort.rules.push({ model: '', effort: 'xhigh' })
  markDirty(id)
}

function removeEffortRule(id: number, index: number) {
  const acc = accounts.value.find((a) => a.id === id)
  if (!acc) return
  acc.config.reasoning_effort.rules.splice(index, 1)
  markDirty(id)
}
